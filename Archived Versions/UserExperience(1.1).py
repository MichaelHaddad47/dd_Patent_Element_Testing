...
- Finds Patent using only the first 6 lines of the XML file (<doc-numer>), this assumes same 
formatting for all XML files (CAREFUL for future use/projects)
        ...
        for filename in os.listdir(xml_dir):
            ...
            with open(file_path, 'r', encoding='utf-8') as file:
                for i, line in enumerate(file):
                    if i > 6:  # Assuming doc-number is always within the first 6 lines
                        break
                    # Check for the doc-number tag in the line
                    if '<doc-number>' in line:
                        # Extract the numeric part of the doc-number tag
                        start = line.find('<doc-number>') + len('<doc-number>')
                        end = line.find('</doc-number>', start)
                        doc_num = line[start:end]
                        # Compare with both zero-padded and non-padded patent number
                        if doc_num == patent_number.zfill(len(doc_num)) or doc_num.lstrip('0') == patent_number.lstrip('0'):
                            return file_path
        return None
...
def main():
    ...
    print(f"\nIt took {end_time - start_time:.2f} seconds to find the patent, and list all elements.")
